...
static LONG findDrvPath (TCHAR *clsidstr, TCHAR *dllpath, int dllpathsize)
{
	...
	CharLowerBuff(clsidstr, lstrlen(clsidstr));
	if ((cr = RegOpenKey(HKEY_CLASSES_ROOT, COM_CLSID, &hkEnum)) == ERROR_SUCCESS)
	{
		...
		while (cr == ERROR_SUCCESS && !found)
		{
			...
			if (cr == ERROR_SUCCESS)
			{
				CharLowerBuff(databuf, lstrlen(databuf));
...
{	
	...
	if ((lpdrv = getDrvStruct(drvID,lpdrvlist)) != 0)
	{
		if (lstrlen(lpdrv->drvname) < drvnamesize)
		{
...
LONG AsioDriverList::asioGetDriverPath(int drvID, TCHAR *dllpath, int dllpathsize)
{
	...
	if ((lpdrv = getDrvStruct(drvID,lpdrvlist)) != 0)
	{
		if (lstrlen(lpdrv->dllpath) < dllpathsize)
		{
