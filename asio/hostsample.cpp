...
{	// collect the informational data of the driver
	...
	{
		g_playerDlg.LogPrint(_T("ASIOGetChannels: inputs=%d, outputs=%d"), asioDriverInfo->inputChannels, asioDriverInfo->outputChannels);
		...
		{
			g_playerDlg.LogPrint(_T("ASIOGetBufferSize: min=%d, max=%d, preferred=%d, granularity=%d"),
			...
			{
				g_playerDlg.LogPrint(_T("ASIOGetSampleRate: sampleRate=%f"), asioDriverInfo->sampleRate);
				...
				g_playerDlg.LogPrint(_T("ASIOOutputReady() - %s"), asioDriverInfo->postOutput ? _T("Supported") : _T("Not supported"));
...
void FillSineWave_Int32LSB(int32_t* pLeft, int32_t* pRight, int len)
{
...
void PlayerDlg::FillAudioData(int32_t* pLeft, int32_t* pRight, int len)
{
	if (m_pMP3 == NULL)
	{
...
{	// create buffers for all inputs and outputs of the card with the 
	...
	if (result == ASE_OK)
	{
		...
		if (result == ASE_OK)
		{
			...
			if (result == ASE_OK)
				g_playerDlg.LogPrint(_T("ASIOGetLatencies: input=%d, output=%d"), asioDriverInfo->inputLatency, asioDriverInfo->outputLatency);
		}
	}
	return result;
}
...
void PlayerDlg::StopAsio()
{
	ASIOStop();
	m_bAsioStarted = false;
	LogPrint(_T("ASIO stopped."));
}

void PlayerDlg::UpdateAsioState()
{
	LogPrint(_T("%d ms / %d ms / %d samples"), asioDriverInfo.sysRefTime, (long)(asioDriverInfo.nanoSeconds / 1000000.0), (long)asioDriverInfo.samples);
	...
	LogPrint(_T(" / TC: %2.2d:%2.2d:%2.2d:%5.5d"), (long)hours, (long)minutes, (long)seconds, (long)remainder);
}
